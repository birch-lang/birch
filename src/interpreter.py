...
from birch_types import TokenType as TT
from visitor import Visitor
from expr import *
from stmt import *
from environment import Environment
from birch_function import BirchFunction
from birch_callable import BirchCallable
from birch_return import BirchReturn
from birch_class import BirchClass
from birch_instance import BirchInstance
from error_handler import BirchRuntimeException
...
class Interpreter(Visitor):
        ...
        if isinstance(x, AssignExpr):
            ...
            if tt == TT.GREATER:
                ...
                if type(left) == type(right):
                    return left + right
                raise(BirchRuntimeException(x.operator, "Operands must be two numbers or two strings."))
            elif tt == TT.SLASH:
                self.checkNumberOperands(x.operator, left, right)
                return left // right
            elif tt == TT.STAR:
            ...
            if not isinstance(callee, BirchCallable):
                raise(BirchRuntimeException(x.paren, "Can only call functions and classes."))
            function = callee
            if len(arguments) != function.arity():
                raise(BirchRuntimeException(x.paren, f"Expected {function.arity()} arguments but got {len(arguments)}."))
            return function.call(self, arguments)
        elif isinstance(x, GetExpr):
            obj = self.evaluate(x.obj)
            if isinstance(obj, BirchInstance):
                return obj.get(x.name)
            raise(BirchRuntimeException(x.name, "Only instances have properties."))
            ...
            obj = self.evaluate(x.obj)
            if not isinstance(obj, BirchInstance):
                raise(BirchRuntimeException(x.name, "Only instances have fields."))
            value = self.evaluate(x.value)
            ...
            if not method:
                raise(BirchRuntimeException(x.method, f"Undefined property '{x.method.lexeme}'."))
            ...
            if x.superclass:
                ...
                if not isinstance(superclass, BirchClass):
                    raise(BirchRuntimeException(x.superclass.name, "Superclass must be a class."))
            ...
            for method in x.methods:
                function = BirchFunction(method, self.environment, method.name.lexeme == "init")
                methods[method.name.lexeme] = function

            klass = BirchClass(x.name.lexeme, superclass, methods)
            ...
            function = BirchFunction(x, self.environment)
    ...
    def interpret(self, statements):
        ...
        except BirchRuntimeException as e:
            self.error_handler.runtimeError(e)

    def checkNumberOperand(self, operator, operand):
        if isinstance(operand, int) or isinstance(operand, float):
            return
        raise(BirchRuntimeException(operator, "Operand must be a number."))
    ...
    def checkNumberOperands(self, operator, left, right):
        ...
        if isinstance(left, float) and isinstance(right, float):
            return
        raise(BirchRuntimeException(operator, "Operands must be numbers."))
