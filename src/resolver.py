...
from stmt import *
from expr import *
from birch_types import FunctionType as FT, ClassType as CT
...
class Resolver(Visitor):
    ...
    def visit(self, x):  # sourcery no-metrics
        if isinstance(x, BlockStmt):
            ...
            if (
                len(self.scopes) > 0
                and x.name.lexeme in self.scopes[-1]
                and self.scopes[-1][x.name.lexeme] == False
            ):
                self.error_handler.error(x.name, "Cannot read local variable in its own initializer.")
            self.resolveLocal(x, x.name)
        return None

    def _extracted_from_visit_7(self, x):
        ...
        if x.superclass and x.name.lexeme == x.superclass.name.lexeme:
